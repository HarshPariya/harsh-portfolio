...
const TintImages = memo(({ opacity }: { opacity: number }) => {
  return (
    <div className="relative h-[60vw] max-h-[400px] w-full rounded-2xl border border-neutral-300 bg-white lg:h-screen">
      {/* No Tint Image (base) */}
      <Image src={heroBefore} alt="No Window Tint" fill className="object-contain" sizes="(max-width: 768px) 90vw, (max-width: 1023px) 50vw" />
      ...
      <MotionDiv initial={{ opacity: 0 }} animate={{ opacity }} transition={{ ease: "easeIn", delay: 0.2 }} className="absolute inset-0">
        <Image src={heroBefore} alt="5% Window Tint" fill className="rounded-4xl object-contain" sizes="(max-width: 768px) 90vw, (max-width: 1023px) 80vw" />
      </MotionDiv>
    </div>
  )
})
